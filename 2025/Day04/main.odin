...
solve_2::#force_no_inline proc(input:[]string)->(result:=0){
   input_copy_1:=slice.clone(transmute([][]u8)input)
   input_copy_2:=slice.clone(transmute([][]u8)input)
   for line,i in input{
      input_copy_1[i]=slice.clone(transmute([]u8)line)
      input_copy_2[i]=slice.clone(transmute([]u8)line)
   }
   changed:=true
   for changed{
      changed=false
      ...
      for y in 0..=maxy{
         ...
         for x in 0..=maxx{
            ...
            }else if input_copy_1[y][x]=='x'{
               input_copy_2[y][x]='x'
            }
         }
      }
      input_copy_1,input_copy_2=input_copy_2,input_copy_1
   }
   for _,i in input{
      delete(input_copy_2[i])
      delete(input_copy_1[i])
   }
   delete(input_copy_2)
   delete(input_copy_1)
   return result
