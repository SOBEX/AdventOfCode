...
solve_2::#force_no_inline proc(input:[]string)->(result:=0){
   for line in input{
      ...
      fmt.println(values,buttons)

      config:=mk_config()
      defer del_config(config)
      c:=mk_context(config)
      defer del_context(c)

      int_sort:=mk_int_sort(c)

      varc:=u32(len(buttons))
      vars:=make([]Ast,varc)
      defer delete(vars)
      for i in 0..<varc{
         name:=fmt.ctprintf("b%i",i)
         symbol:=mk_string_symbol(c,name)
         vars[i]=mk_const(c,symbol,int_sort)
      }
      defer free_all(context.temp_allocator)

      optimizer:=mk_optimize(c)
      optimize_inc_ref(c,optimizer)
      defer optimize_dec_ref(c,optimizer)
      ...
      for i in 0..<length{
         ...
         value:=i32(values[i])
         optimize_assert(c,optimizer,mk_eq(c,mk_add(c,argc,slice.as_ptr(args)),mk_int(c,value,int_sort)))
      }

      optimize_minimize(c,optimizer,mk_add(c,varc,slice.as_ptr(vars)))

      for i in 0..<varc{
         optimize_assert(c,optimizer,mk_ge(c,vars[i],mk_int(c,0,int_sort)))
      }

      if optimize_check(c,optimizer,0,nil)==.True{
         model:=optimize_get_model(c,optimizer)
         if model!=nil do model_inc_ref(c,model)
         defer if model!=nil do model_dec_ref(c,model)

         sum:=0
         for i in 0..<varc{
            ast:Ast
            var:i32
            if model_eval(c,model,vars[i],true,&ast)&&get_numeral_int(c,ast,&var){
               fmt.printf("var[%i]=%i,",i,var)
               sum+=int(var)
            }
         }
         fmt.println()

         fmt.println("Solvable:",sum)
         result+=sum
      }else{
