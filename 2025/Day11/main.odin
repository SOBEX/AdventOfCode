...
solve_1::#force_no_inline proc(input:[]string)->(result:=0){
   device_outputs:map[string][]string
   defer delete(device_outputs)
   defer for device,outputs in device_outputs do delete(outputs)
   for line in input{
      line:=line
      device:=strings.split_iterator(&line,": ") or_continue
      outputs:=strings.split(line," ") or_continue
      device_outputs[device]=outputs
   }
   devices:[dynamic]string
...
solve_2::#force_no_inline proc(input:[]string)->(result:=0){
   ...
   defer delete(states)
   states[State{"svr",false,false}]=1
   for len(states)>0{
      new_states:map[State]int
