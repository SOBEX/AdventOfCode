...
solve_1::proc(input:Input)->int{
   ...
   for line in input{
      good:=0
      i:=0
      for i:=0;i-good<len(line)-3;i+=1{
         if strings.index_byte(line[i+1:i+4-good],line[i])==-1{
            good+=1
            if good==3{
               sum+=i+2
               break
...
solve_2::proc(input:Input)->int{
   ...
   for line in input{
      good:=0
      i:=0
      for i:=0;i-good<len(line)-13;i+=1{
         if strings.index_byte(line[i+1:i+14-good],line[i])==-1{
            good+=1
            if good==13{
               sum+=i+2
               break
