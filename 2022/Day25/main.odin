...
solve_1::proc(input:[]string)->(result:=0){
   ...
   remaining:=result
   for ;comp>0;comp/=5{
      best:=[3]int{max(int),0,0}
      if abs(remaining+comp*2)<best[0] do best=[3]int{abs(remaining+comp*2),remaining+comp*2,-2}
      if abs(remaining+comp*1)<best[0] do best=[3]int{abs(remaining+comp*1),remaining+comp*1,-1}
      if abs(remaining       )<best[0] do best=[3]int{abs(remaining       ),remaining       , 0}
      if abs(remaining-comp*1)<best[0] do best=[3]int{abs(remaining-comp*1),remaining-comp*1, 1}
      if abs(remaining-comp*2)<best[0] do best=[3]int{abs(remaining-comp*2),remaining-comp*2, 2}
      remaining=best[1]
      fmt.print(rune(chars[best[2]+2]))
   }
   fmt.println()
   return result
}
