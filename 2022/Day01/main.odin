...
solve_2::proc(input:Input)->int{
   best:pq.Priority_Queue(int)
   less::proc(l,r:int)->bool{
      return l>r
   }
   swap::proc(q:[]int,l,r:int){
      tmp:=q[l]
      q[l]=q[r]
      q[r]=tmp
   }
   pq.init(&best,less,swap)
   defer pq.destroy(&best)
   sum:int=0
   for line in input{
      if len(line)==0{
         pq.push(&best,sum)
   ...
   pq.push(&best,sum)
   return pq.pop(&best)+pq.pop(&best)+pq.pop(&best)
}
